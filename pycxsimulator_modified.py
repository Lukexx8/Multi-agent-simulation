...
class GUI:
    ...
    # --------------------------------------
    def G__init__graph(self, N, k, seed = None):
        # tworzenie grafu
        self.g = nx.watts_strogatz_graph(N, k, 0.2, seed=seed)
        ...
        for u, v in self.g.edges():
            self.g[u][v]['weight'] = np.random.uniform(0, 0.1)
        ...
        nx.set_node_attributes(self.g, 'immunity', {n : np.random.uniform(0, 1) for n in self.g.nodes_iter()})
        ...
        # ile procent poczatkowo zainfekowac <-----------------------
        self.G_infect_random(0.3)
    
    # na poczatku losowo infekujemy okreslony procent agentow     
    def G_infect_random(self, infect_perc):
    ...
    def G_simulation_step(self):
        for n in self.g.nodes_iter():
            ...
            if self.g.node[n]['state'] == 'infected':
                ...
                prob_of_death = np.random.exponential(0.1) * (1 - self.g.node[n]['immunity'])
                ...
                if np.random.uniform(0, 1) < prob_of_death:
                    ...
                    if np.random.uniform(0, 1) < prob_of_heal:
                        ...
                        self.g.node[n]['immunity'] = min(1, self.g.node[n]['immunity'] * 1.3)
            
            # jezeli agent mimo zarazenia nie umarl ani nie wyzdrowial (jest wciaz zarazony)   
            # zaraza wierzcholki z ktorymi jest polaczony jezeli dojdzie do spotkania 
            # (prawdopodobienstwo spotkania zdefiniowane jest w krawedzi), 
            # prawdopodobienstwo zarazenia skorygowane jest o indywidualna odpornosc
            if self.g.node[n]['state'] == 'infected':
    ...
    def start(self,func=[]):
        if len(func)==3:
            self.modelInitFunc = self.G__init__graph
            self.modelDrawFunc = self.G_draw
            self.modelStepFunc = self.G_simulation_step
